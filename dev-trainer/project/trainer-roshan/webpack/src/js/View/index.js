...
export default class View {
    ...
    render(el) {
        this.onBeforeRender()
        const block = $('<div></div>') // const block = document.createElement('div')
        block.html(this.template(this.templateContext())) // block.innerHTML = this.template(this.templateContext())
        $(el).empty() // el.innerHTML = ''
        $(el).append(block) // el.appendChild(block)
        this.$el = $($(el).find('div')[0]) // this.$el = el.querySelector('div')
        ...
        try {
            eventKeys.forEach(key => {
                if((key[0] === '.' || key[0] === '#') && key[1] && typeof(events[key]) === 'function' && key.split(' ').length === 2) {
                    ...
                    const element = this.$el.find(selector) // const element = this.$el.querySelectorAll(selector)
                    // Array.from(element).forEach(el => {
                    //     el.addEventListener(eventType, events[key])
                    // })
                    element.each((ind, el) => {
                        $(el).on(eventType, events[key])
                    })
                }
            })
        } catch(e) {
    ...
    attachAttributes(attrs) {
        ...
        attributes.forEach((attribute, ind) => {
            ...
            this.$el.attr(attribute, values[ind])
        })
    }
